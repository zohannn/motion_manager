...
input_str = str(sys.argv[4]).split(",")  # string with the given new situation
...
#l1_tp_model_unfit = VSMModel(X=D_prime_dataset_df_clipped.iloc[:,:-2], Y=D_prime_dataset_df_clipped.iloc[:,-2:], n_D=dim_cold, weights_init=w_init, r=r_init)
#l1_tp_model_opt = VSMModel(X=D_prime_dataset_df_clipped.iloc[:,:-2], Y=D_prime_dataset_df_clipped.iloc[:,-2:], n_D=dim_cold, weights_init=w_values, r=r_value)
l1_tp_model_unfit = VSMModel(n_D=dim_cold, weights_init=w_init, r_init=r_init)
l1_tp_model_opt = VSMModel(n_D=dim_cold, weights_init=w_values, r_init=r_value)
...
#print(qi_pred_rdm)
#print(x_new)
qi_pred_unfit = l1_tp_model_unfit.predict_init(x_new, inputs_dataframe_df_scaled, init_cold_dataframe)
qi_pred_opt = l1_tp_model_opt.predict_init(x_new, inputs_dataframe_df_scaled, init_cold_dataframe)
...
for i in range(0,len(qi_dual_bounce)):
    pred_file.write("%.15f" % qi_dual_bounce[i])
    if not (i == (len(qi_dual_bounce) -1)):
        pred_file.write("|")
pred_file.write("\n")
