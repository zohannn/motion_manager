...
#sys.exit("OK") ### stop here

maxiter = 5000 # max iterations
# default initialization
w_init = 0.0001 * np.ones(f_dim)
r_init = 0.6
w_init_opt = 0.0001 * np.ones(f_dim)
r_init_opt = 0.6
# regularization
reg_w = 0.0001
reg_r = 0.1
...
X = D_prime_dataset_df_clipped.iloc[:,:-n_out].copy()
Y = D_prime_dataset_df_clipped.iloc[:,-n_out:].copy()

vsm_model_unfit = VSMModel(X=X, Y=Y,n_D=dim_cold,weights_init=w_init, r_init=r_init,reg_w=reg_w,reg_r=reg_r)
vsm_model_opt = VSMModel(X=X, Y=Y,n_D=dim_cold,weights_init=w_init_opt, r_init=r_init_opt,reg_w=reg_w,reg_r=reg_r)

if check_loss:
    loss_unfit = vsm_model_unfit.loss_function(X,Y, dim_cold, vsm_model_unfit.M,w_init, r_init)
    ...
    loss_opt = vsm_model_opt.loss_function(X,Y, dim_cold, vsm_model_opt.M,w_init_opt, r_init_opt)
    ...
    if res.success:
        ...
        print("Untrained Loss: {}".format(loss_unfit),file=file_untrain_loss)
        file_untrain_loss.close()
        loss_opt = vsm_model_opt.loss_function(X,Y, dim_cold, vsm_model_opt.M,w_best, r_best)
        file_train_loss = open(results_dir+"/train_loss.txt","w")
        print("Trained Loss: {}".format(loss_opt),file=file_train_loss)
        file_train_loss.close()
        print("Untrained Loss: {}. Trained Loss: {}.".format(loss_unfit,loss_opt))
