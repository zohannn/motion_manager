            ...
            if (len(cl_out_dual_bounce_df.index) > min_cluster_size_dual_bounce):
                if (len(cl_out_dual_bounce_df.columns) > 10):
                    ...
                    if (print_en_dual_bounce):
                        ...
                        threedee_train = plt.figure().gca(projection='3d')
                        threedee_train.scatter(cl_in_dual_bounce_df["target_x_mm"], cl_in_dual_bounce_df["target_y_mm"], pc_df['dual_bounce_288'])
                        plt.clf()
                        #plt.show()
                else:
                ...
                fig = plt.figure()
                ax1 = fig.add_subplot(111)
                ax1.scatter(denorm_test_targets["dual_bounce_288"], denorm_test_targets["dual_bounce_354"], s=10, c='b', marker="s", label='test_targets')
                ax1.scatter(denorm_test_predictions_df["dual_bounce_288"], denorm_test_predictions_df["dual_bounce_354"], s=10, c='r', marker="o", label='test_predictions')
                plt.xlabel("dual_bounce_287")
                plt.ylabel("dual_bounce_354")
                plt.legend(loc='upper right')
