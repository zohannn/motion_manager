#include "../include/motion_manager/rrtstardialog.hpp"

namespace motion_manager {

RRTstarDialog::RRTstarDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::RRTstarDialog)
{
    ui->setupUi(this);
    ...
    this->config = std::string("RRTstarkConfigDefault");
}

RRTstarDialog::~RRTstarDialog()
{
    delete ui;
}

void RRTstarDialog::getPreGraspApproach(std::vector<double> &pre_grasp)
{
    pre_grasp.clear();
...
void RRTstarDialog::getPostGraspRetreat(std::vector<double> &post_grasp)
{
    post_grasp.clear();
...
void RRTstarDialog::getPrePlaceApproach(std::vector<double> &pre_place)
{
    pre_place.clear();
...
void RRTstarDialog::getPostPlaceRetreat(std::vector<double> &post_place)
{
    post_place.clear();
...
std::string RRTstarDialog::getConfig()
{
    ...
    if(ui->radioButton_1->isChecked()){
        this->config = std::string("RRTstarkConfig1");
    }
    if(ui->radioButton_2->isChecked()){
        this->config = std::string("RRTstarkConfig2");
    }
    if(ui->radioButton_3->isChecked()){
        this->config = std::string("RRTstarkConfig3");
    }
...
void RRTstarDialog::getFinalHand(std::vector<double> &finalHand)
{
    finalHand.clear();
...
void RRTstarDialog::setPlaneParameters(std::vector<double> &point1,std::vector<double> &point2,std::vector<double> &point3)
{
...
void RRTstarDialog::getPlaneParameters(std::vector<double> &params,
...
bool RRTstarDialog::getApproach()
{
    return !ui->checkBox_approach->isChecked();
}

bool RRTstarDialog::getRetreat()
{
...
void RRTstarDialog::on_pushButton_save_clicked()
{
    ...
    if(f.open( QIODevice::WriteOnly )){
        ...
        stream << "### Parameters of the RRT Star planner in MoveIt! ###" << endl;
...
void RRTstarDialog::on_pushButton_load_clicked()
{
    ...
    if(f.open( QIODevice::ReadOnly )){
        ...
        if(strcmp(this->config.c_str(),"RRTstarkConfigDefault")==0){
            ui->radioButton_default->setChecked(true);
        }else if(strcmp(this->config.c_str(),"RRTstarkConfig1")==0){
            ui->radioButton_1->setChecked(true);
        }else if(strcmp(this->config.c_str(),"RRTstarkConfig2")==0){
            ui->radioButton_2->setChecked(true);
        }else if(strcmp(this->config.c_str(),"RRTstarkConfig3")==0){
            ui->radioButton_3->setChecked(true);
        }
...
void RRTstarDialog::setInfo(std::string info)
{

    this->infoLine = info;
}

void RRTstarDialog::checkFinalPosture(int state)
{
    if(state==0){
        // unchecked
...
void RRTstarDialog::checkAddPlane(int state)
{
    if(state==0){
        //unchecked
...
bool RRTstarDialog::get_use_final_posture()
{
    return ui->checkBox_sel_final_posture->isChecked();
}

bool RRTstarDialog::get_add_plane()
{
...
void RRTstarDialog::checkApproach(int state)
{
    if(state==0){
        // unchecked
...
void RRTstarDialog::checkRetreat(int state)
{
    if(state==0){
        // unchecked
