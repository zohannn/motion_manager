#include "../include/motion_manager/prmdialog.hpp"

namespace motion_manager {

PRMDialog::PRMDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::PRMDialog)
{
    ui->setupUi(this);
...
void PRMDialog::getPreGraspApproach(std::vector<double> &pre_grasp)
{
    pre_grasp.clear();
...
void PRMDialog::getPostGraspRetreat(std::vector<double> &post_grasp)
{
    post_grasp.clear();
...
void PRMDialog::getPrePlaceApproach(std::vector<double> &pre_place)
{
    pre_place.clear();
...
void PRMDialog::getPostPlaceRetreat(std::vector<double> &post_place)
{
    post_place.clear();
...
std::string PRMDialog::getConfig()
{
    ...
    if(ui->radioButton_1->isChecked()){
        this->config = std::string("PRMkConfig1");
    }
    if(ui->radioButton_2->isChecked()){
        this->config = std::string("PRMkConfig2");
    }
...
void PRMDialog::setTargetMove(std::vector<double> &target)
{
...
void PRMDialog::setPlaneParameters(std::vector<double> &point1,std::vector<double> &point2,std::vector<double> &point3)
{
...
bool PRMDialog::getApproach()
{
    return !ui->checkBox_approach->isChecked();
}

bool PRMDialog::getRetreat()
{
...
void PRMDialog::on_pushButton_save_clicked()
{
    ...
    if(f.open( QIODevice::WriteOnly )){
        ...
        stream << "### Parameters of the PRM planner in MoveIt! ###" << endl;
...
void PRMDialog::on_pushButton_load_clicked()
{
    ...
    if(f.open( QIODevice::ReadOnly )){
        ...
        if(strcmp(this->config.c_str(),"PRMkConfigDefault")==0){
            ui->radioButton_default->setChecked(true);
        }else if(strcmp(this->config.c_str(),"PRMkConfig1")==0){
            ui->radioButton_1->setChecked(true);
        }else if(strcmp(this->config.c_str(),"PRMkConfig2")==0){
...
void PRMDialog::checkApproach(int state)
{
    if(state==0){
        // unchecked
...
void PRMDialog::checkRetreat(int state)
{
    if(state==0){
        // unchecked
